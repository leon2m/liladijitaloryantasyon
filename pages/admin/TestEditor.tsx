        ...
        <form onSubmit={handleSubmit} className="fade-in space-y-10">
            ...
            <div className="glass-card p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label htmlFor="testName" className="block text-sm font-medium text-gray-700 mb-1">Test Adı</label>
                        <input id="testName" name="name" value={test.name} onChange={handleInputChange} className="input-style" />
                    </div>
                    <div>
                        <label htmlFor="testDuration" className="block text-sm font-medium text-gray-700 mb-1">Süre</label>
                        <input id="testDuration" name="duration" value={test.duration} onChange={handleInputChange} className="input-style" />
                    </div>
                    <div className="md:col-span-2">
                        <label htmlFor="testDescription" className="block text-sm font-medium text-gray-700 mb-1">Açıklama</label>
                        <textarea id="testDescription" name="description" value={test.description} onChange={handleInputChange} className="input-style h-24" />
                    </div>
                </div>
            </div>
            ...
            <div className="glass-card p-6">
                ...
                <div className="space-y-4">
                    {Object.entries(test.resultProfiles).map(([key, profile], pIndex) => (
                        <div key={key} className="grid grid-cols-1 md:grid-cols-8 gap-4 items-center bg-gray-50/50 p-3 rounded-lg">
                            <input aria-label={`Profil ${pIndex+1} Adı`} value={profile.name} onChange={(e) => handleProfileChange(key, 'name', e.target.value)} placeholder="Profil Adı" className="input-style md:col-span-2"/>
                            <textarea aria-label={`Profil ${pIndex+1} Açıklaması`} value={profile.description} onChange={(e) => handleProfileChange(key, 'description', e.target.value)} placeholder="Açıklama" className="input-style md:col-span-4 h-12"/>
                            <input aria-label={`Profil ${pIndex+1} Rengi`} type="color" value={profile.color} onChange={(e) => handleProfileChange(key, 'color', e.target.value)} className="input-style h-12 md:col-span-1"/>
                ...
                <div className="space-y-8">
                    {test.questions.map((q, qIndex) => (
                        <div key={q.id} className="bg-gray-50/50 p-4 rounded-lg border">
                             <label htmlFor={`q-text-${qIndex}`} className="block text-sm font-medium text-gray-700 mb-1">Soru {qIndex + 1}</label>
                            <div className="flex justify-between items-start mb-4">
                                <textarea id={`q-text-${qIndex}`} value={q.text} onChange={(e) => handleQuestionTextChange(qIndex, e.target.value)} className="input-style w-full mr-4"/>
                                ...
                                {q.options.map((opt, oIndex) => (
                                    <div key={oIndex} className="bg-white p-3 rounded-md border">
                                        <div className="flex items-center mb-2">
                                            <label htmlFor={`q-${qIndex}-opt-${oIndex}`} className="sr-only">Soru {qIndex+1} Seçenek {oIndex+1}</label>
                                            <input id={`q-${qIndex}-opt-${oIndex}`} value={opt.text} onChange={(e) => handleOptionTextChange(qIndex, oIndex, e.target.value)} placeholder="Seçenek metni" className="input-style w-full mr-2"/>
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 pt-2 border-t">
                                            {Object.entries(test.resultProfiles).map(([key, profile]) =>(
                                                <div key={key} className="flex items-center">
                                                    <label htmlFor={`q-${qIndex}-opt-${oIndex}-score-${key}`} className="text-sm mr-2 whitespace-nowrap" style={{color: profile.color || '#000'}}>{profile.name}:</label>
                                                    <input id={`q-${qIndex}-opt-${oIndex}-score-${key}`} type="number" value={opt.scores[key] || ''} onChange={e => handleScoreChange(qIndex, oIndex, key, e.target.value)} placeholder="Puan" className="input-style w-full !py-1"/>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
